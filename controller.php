...
if ($_POST['action'] == 'newsletter') {
    ...
    $checkemail = $conn->query("SELECT * FROM r_subscriber where email='" . $email . "' ");
    ...
    if ($emailCount == 0) {
        $subquery = $conn->query("INSERT INTO r_subscriber (email)values('" . $email . "')");
        if ($subquery) {
...
if ($_POST['action'] == 'contact') {
    ...
    $query = $conn->query("INSERT INTO r_message (subject,name,email,message)values('" . $subject . "','" . $name . "','" . $email . "','" . $message . "')");
    if ($mail) {
